...
<head>
    <meta charset="UTF-8">
    <title><?php echo ($meta_title); ?>|第一枪管理平台</title>
    <link href="/Public/login.ico" type="image/x-icon" rel="shortcut icon">
...
<!--                <li><a href="<?php echo U('User/updateNickname');?>">修改昵称</a></li>-->
		...
		<ul class="main-nav" style="float:right;margin-right:20px;">
...
<script>
	...
	<div class="main-title">
		...
		<?php if(!empty($rightNav)): if(is_array($rightNav)): $i = 0; $__LIST__ = $rightNav;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nav): $mod = ($i % 2 );++$i;?><a href="<?php echo U('mydocument','cate_id='.$nav['id']);?>"><?php echo ($nav["title"]); ?></a>
		...
		<?php else: ?>
		<?php if(empty($position)): ?>全部<?php else: ?><a href="<?php echo U('article/mydocument');?>">全部</a><?php endif; ?>
			<?php if(is_array(C("DOCUMENT_POSITION"))): foreach(C("DOCUMENT_POSITION") as $key=>$vo): if(($position) != $key): ?><a href="<?php echo U('mydocument',array('position'=>$key));?>"><?php echo ($vo); ?></a><?php else: echo ($vo); endif; ?>&nbsp;<?php endforeach; endif; endif; ?>
		]
		<?php if(($allow) == "0"): ?>（该分类不允许发布内容）<?php endif; ?>
		<?php if(count($model) > 1): ?>[ 模型：<?php if(empty($model_id)): ?><strong>全部</strong><?php else: ?><a href="<?php echo U('mydocument',array('pid'=>$pid,'cate_id'=>$cate_id));?>">全部</a><?php endif; ?>
			<?php if(is_array($model)): $i = 0; $__LIST__ = $model;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(($model_id) != $vo): ?><a href="<?php echo U('mydocument',array('pid'=>$pid,'cate_id'=>$cate_id,'model_id'=>$vo));?>"><?php echo (get_document_model($vo,'title')); ?></a><?php else: ?><strong><?php echo (get_document_model($vo,'title')); ?></strong><?php endif; ?>&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
		]<?php endif; ?>
		<?php if(!empty($groups)): ?>[ 分组：<?php if(empty($group_id)): ?><strong>全部</strong><?php else: ?><a href="<?php echo U('mydocument',array('pid'=>$pid,'cate_id'=>$cate_id));?>">全部</a><?php endif; ?>
			<?php if(is_array($groups)): foreach($groups as $key=>$vo): if(($group_id) != $key): ?><a href="<?php echo U('mydocument',array('pid'=>$pid,'cate_id'=>$cate_id,'model_id'=>$model_id,'group_id'=>$key));?>"><?php echo ($vo); ?></a><?php else: ?><strong><?php echo ($vo); ?></strong><?php endif; ?>&nbsp;<?php endforeach; endif; ?>
		]<?php endif; ?>
		</h2>
	</div>
    ...
    <tbody>
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; $pos = getPosition($vo['position']); $show_pos = ''; if (array_key_exists("s", $pos)) { $show_pos .= '<span style="color:#fff;font-size:12px; border-radius: 3px;padding: 0 2px;margin-left:5px; background-color:#fb2b03;">首</span>'; } if (array_key_exists("p", $pos)) { $show_pos .= '<span style="color:#fff;font-size:12px; border-radius: 3px;padding: 0 2px;margin-left:5px; background-color:#FF9900;">频</span>'; } if (array_key_exists("l", $pos)) { $show_pos .= '<span style="color:#fff;font-size:12px; border-radius: 3px;padding: 0 2px;margin-left:5px; background-color:#FFCC00;">列</span>'; } ?>
		<tr>
            <td><input class="ids" type="checkbox" name="ids[]" value="<?php echo ($vo["id"]); ?>" /></td>
			<td><?php echo ($vo["id"]); ?> </td>
			<td><a href="<?php echo U('Article/edit?cate_id='.$vo['category_id'].'&id='.$vo['id']);?>"><?php echo ($vo["title"]); echo ($show_pos); ?></a></td>
    ...
    <div class="page">
        ...
        <div class="cont-ft">
            <div class="copyright">
                <div class="fl">感谢使用<a href="http://www.diyiqiang.cn" target="_blank">第一枪</a>管理平台</div>
