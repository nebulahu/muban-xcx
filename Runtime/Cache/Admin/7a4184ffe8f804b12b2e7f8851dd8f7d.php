...
<script>
	...
	<div class="main-title">
		<h2>
		<!--文档列表-->
		<?php if(!empty($rightNav)): if(is_array($rightNav)): $i = 0; $__LIST__ = $rightNav;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nav): $mod = ($i % 2 );++$i; if(count($rightNav) == $i): ?><a href="<?php echo U('index','cate_id='.$nav['id']);?>"><?php echo ($nav["title"]); ?></a><?php endif; endforeach; endif; else: echo "" ;endif; ?>
		<?php else: ?>
		文档列表<?php endif; ?>
		(<?php echo ($_total); ?>) 
		[
		<?php if(!empty($rightNav)): ?><!--<?php if(is_array($rightNav)): $i = 0; $__LIST__ = $rightNav;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nav): $mod = ($i % 2 );++$i;?><a href="<?php echo U('index','cate_id='.$nav['id']);?>"><?php echo ($nav["title"]); ?></a>
			<?php if(count($rightNav) > $i): ?><i class="ca"></i><?php endif; endforeach; endif; else: echo "" ;endif; ?>-->
		<?php if($cate_id != 0): if(is_array(C("DOCUMENT_POSITION"))): foreach(C("DOCUMENT_POSITION") as $key=>$vo): if(($position) != $key): ?><a href="<?php echo U('index',array('cate_id'=>$cate_id,'position'=>$key));?>"><?php echo ($vo); ?></a><?php else: echo ($vo); endif; ?>&nbsp;<?php endforeach; endif; endif; ?>    
