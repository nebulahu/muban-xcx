...
class Article extends TagLib{
    ...
    protected $tags   =  array(
		...
		'pagewap'     => array('attr' => 'cate,listrow', 'close' => 0), //列表分页
...
//        $field  = empty($tag['field']) ? 'true' : $tag['field'];
    ...
    public function _position($tag, $content){
		...
		if(!empty($pid)){
			$parse    = '<?php ';
			$parse   .= '$ids = "";$info = D(\'Category\')->getInfo(';
			$parse   .= $pid . ',"id");foreach($info as $v):$ids .= $v["id"].","; endforeach;$ids = trim($ids,",");?>'; 
    ...
    public function _page($tag){
        ...
        $listrow = $tag['listrow'];
        $parse   = '<?php ';
        $parse  .= '$__PAGE__ = new \Think\Pageweb(get_list_count(' . $cate . '), ' . $listrow . ');';
    ...
    public function _pagewap($tag){
        ...
        $parse   = '<?php ';
        $parse  .= '$__PAGE__ = new \Think\Pagewap(get_list_count(' . $cate . '), ' . $listrow . ');';
