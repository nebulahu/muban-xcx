...
<div class="tab-wrap">
    ...
    <!-- 表单 -->
    <form id="form" action="<?php echo U('edit?name='.trim($_GET['name']));?>" method="POST" class="form-horizontal">
        <!-- 基础文档模型 -->
		<div id="tab" class="tab-pane in tab1">
            <?php if(is_array($fields)): $i = 0; $__LIST__ = $fields;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i; if($field['is_show'] == 1 || ($field['is_show'] == 3 && $data['id']) || ($field['is_show'] == 2 && !$data['id'])): ?><div class="form-item cf">
            ...
            <input type="hidden" name="id" value="<?php echo ((isset($data["id"]) && ($data["id"] !== ""))?($data["id"]):0); ?>">
        </div>
