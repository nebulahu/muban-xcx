...
/**    public function updateNickname(){
	...
	public function changeaudit(){
		...
		$id = is_array($id) ? implode(',',$id) : $id;
		//dump($id);exit;
		$info = M('ucenter_member');
		$data = array('id'=>$id,'audit'=>1);
		//dump($data);exit;
		$res = $info->save($data);
		if($res){
			$this->success('审核成功！');
    ...
    public function add($username = '', $password = '', $repassword = ''){
        if(IS_POST){
            ...
            if(0 < $uid){ //注册成功
                $user = array('uid' => $uid,'cname' => $cname, 'nickname' => $username, 'status' => 1);
	...
	public function edit(){
		...
		$ylist = $member->where('type=1 and status=1')->select();
	...
	public function do_edit(){
		...
		/*}else{
			$data['password']=$this->think_ucenter_md5($password, UC_AUTH_KEY);
			//dump($data);exit;
			$result = $member->save($data);
			//dump($result);exit;
			
			$user = array('uid'=>$uid,'nickname'=>$username,'status'=>1);
			$userinfo = M('member')->save($user);
			if($userinfo || $result){
				$this->success('修改成功！',U('index'));
	...
	public function change(){
		$id = I('get.uid');
		D('Home/Member')->adminlogin($id);
		//dump($_SESSION['onethink_home']);exit;
		//dump($_SESSION);exit;
		//echo "<script>window.open('".C('SITE_URL')."', 'newwindow')";
		//dump(C('SITE_URL'));exit;
		redirect(C('SITE_URL'));
	}
}