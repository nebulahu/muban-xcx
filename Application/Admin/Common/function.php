    ...
    if(!empty($grid['href'])){
        ...
        foreach($links as $link){
            ...
            if(preg_match('/^\[([a-z_]+)\]$/',$href,$matches)){
                ...
                // 替换系统特殊字符串
                $href   =   str_replace(
                    array('[DELETE]','[EDIT]','[RESPOND]','[ADDON]'),
                    array('del?ids=[id]&name=[ADDON]','edit?id=[id]&name=[ADDON]','gid?gid=[gid]&id=[id]',$addon),
                    $href);
...
//获取推荐位的值
/*function getPosition($num,$value){
	if(!empty($num)){
		if(is_numeric($num)){
			if($num==3){
