            ...
            <?php if(is_array($fields[$key])): $i = 0; $__LIST__ = $fields[$key];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i; if($field['is_show'] == 1 || $field['is_show'] == 3): ?><div class="form-item cf">
                    ...
                    <div class="controls">
                        <?php switch($field["type"]): case "num": ?><input type="text" class="text input-mid" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"><?php break;?>
                            ...
                            <?php case "uploadpics": ?><!-- 批量上传 --> 
                                 <input type="file" id="upload_all"> 
                                 <input type="hidden" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>" class="input_upload_all"/>
                                 <div class="uploadpics_piclist">
                                 <ul class="highslide-gallery">
                                 <?php if(!empty($data[$field['name']])): $pics=explode(',', $data[$field['name']]); if(!empty($data['pictrue_name'])){ $pictrue_names=explode('&', $data['pictrue_name']); } foreach($pics as $k=>$v){ if($v!=''){ ?> 
                                 <li class="uploadpics-style"><a href="<?php echo (get_cover($v,'path')); ?>" target="_blank"><img src="<?php echo (get_cover($v,'path')); ?>"/></a><input type="text" class="textinput" value="<?php echo ($pictrue_names[$k]); ?>" /><span onclick="delthispic(this)" dataid="<?php echo ($v); ?>,">删除一张</span></li>
                                 <?php
 } } endif; ?>
                                 </ul>
                                 </div>
                                 ...
                                 if(data.status){ 
                                 src = data.url || '' + data.path; 
                                 var value = '<li class="uploadpics-style"><a href="'+src+'" class="highslide" onClick="return hs.expand(this)"><img src="' + src + '"/></a><input type="text" class="textinput" value="" /><span onclick="delthispic(this)" dataid="'+data.id+',">删除一张</span></li>'; 
