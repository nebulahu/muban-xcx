...
function U($url='',$vars='',$suffix=true,$domain=false) {	
    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        ...
        if(C('URL_ROUTER_ON') && !empty($routes) && (MODULE_NAME == 'Home' || MODULE_NAME=='Mobile')){ //如果开启路由而且路由规则不为空
...
//                        $rule = preg_replace("/(\(\\\[\w]\+\))/", $_url[$key], $rule, 1);
					   ...
					   //dump($key);
						//dump($_url[$key]);
						if($_url[$key] === NULL || $url[$_key] === ''){
							$rule = preg_replace("/\&{$key}=/", '', $rule, 1);
						}
                        $rule = preg_replace("/\([^\)]*\)/", $_url[$key], $rule, 1);
...
//                当没有符合的路由规则时 使用兼容模式----- MoLiBei
    ...
    if(C('URL_ROUTER_ON') && !empty($routes) && (MODULE_NAME == 'Home' || MODULE_NAME=='Mobile')){
	   $url =	str_replace("?&","?",$url);
    	if (preg_match("/\?/", $url)) {
...
{
//    require_once('ThinkPHP\Library\Vendor\phpthumb\PhpThumbFactory.class.php');exit;
    define('UPLOAD_URL', '');
    define('UPLOAD_PATH', '');
    ...
    if (!file_exists(UPLOAD_PATH . $oldFile)) {
        ...
        if ($old_image_width <= $width && $old_image_height <= $height) {
            ...
            //生成缩略图 - 更好的方法
            if ($height == "auto"){
                $a = round($old_image_width/$width);
                $height = round($old_image_height/$a);
                unset($a);
            }
//            return $height;
